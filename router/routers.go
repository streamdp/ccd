...
func InitRouter(r *gin.Engine) (err error) {
	...
	apiGroupV1 := r.Group("/v1")
	{
		...
		apiGroupV1.POST("/price", handlers.GinHandler(v1.GetPrice(wc, db)))
		apiGroupV1.GET("/price", handlers.GinHandler(v1.GetPrice(wc, db)))
		apiGroupV1.GET("/ws", ws1.ServeWs(hub))
	}
